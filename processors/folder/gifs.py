#!/usr/bin/env python3
"""
GIF Generator - Folder Processor
Recursively finds all video files and generates animated GIFs (10% of video length).

Usage:
    python gifs.py <folder_path> [--continue-on-error]

Output:
    - gifs/FILENAME.gif for each video
    - logs/gifs.log with processing details
"""
...
def setup_logging(log_dir):
    ...
    log_file = log_dir / 'gifs.log'
    
    # Create logger
    logger = logging.getLogger('gifs_processor')
...
def main():
    """Main entry point for folder GIF generation."""
    if len(sys.argv) < 2:
        print("Usage: python gifs.py <folder_path> [--continue-on-error]")
        sys.exit(1)
    ...
    logger.info("="*80)
    logger.info("GIF GENERATION - FOLDER PROCESSOR")
    logger.info("="*80)
    ...
    file_processor_path = script_dir / 'file' / 'gifs.py'
